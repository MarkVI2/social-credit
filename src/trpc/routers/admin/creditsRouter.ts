...
import { buildAdminGiveMessage, buildAdminTakeMessage } from '@/services/messageTemplates';
...
export const creditsRouter = createTRPCRouter({
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      try {
        ...
        await logTransaction({
          from: amount > 0 ? (sourceAccount === 'admin' ? me.username || me.email || '' : 'classBank') : (target.username || target.email || ''),
          to: amount > 0 ? (target.username || target.email || '') : (sourceAccount === 'admin' ? me.username || me.email || '' : 'classBank'),
        ...
        return {
          success: true,
          message: `Successfully ${amount > 0 ? 'granted' : 'deducted'} ${Math.abs(amount)} credits`,
        };
      } finally {
  ...
  getSystemBalances: adminProcedure.query(async () => {
    try {
      ...
      const totalUserCredits = await users.aggregate([
    ...
    .mutation(async ({ input, ctx }) => {
      try {
        ...
        await recordActivity({
          ...
          message: `Admin ${ctx.user.username || ctx.user.email} reset all user credits to ${input.newAmount}. Reason: ${input.reason}`,
        });
